...
export function AccountPanel() {
  ...
  const { data: reputation } = useProviderReputation(address);
  const completed = reputation?.completed ?? 0;
    ...
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      ...
      <DialogContent className="brand-card border-2">
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="brand-card p-4 space-y-2">
            <p className="text-sm text-muted-foreground">Orders delivered as provider</p>
            <p className="text-2xl font-bold text-accent">{completed}</p>
            {reputation && reputation.rejected > 0 && (
              <p className="text-xs text-muted-foreground">{reputation.rejected} rejected</p>
            )}
          </div>
